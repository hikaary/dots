...
value = (
    args.browser or args.filemanager or args.editor or args.terminal or args.get
)

u = os.path.expanduser

CONFIG_FILE = u('~/.config/hypr/conf/apps.conf')
JSON_CONFIG_FILE = u('~/.config/.settings/apps.json')
...
def replace_value(env_str, key, new_value):
    pattern = re.compile(
        r'^(\s*env\s*=\s*' + re.escape(key) + r'\s*,\s*)(.*?)\s*(#.*)?$',
        re.MULTILINE,
    )
